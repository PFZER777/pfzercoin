...
CzpfzrWallet::CzpfzrWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zpfzr seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZPFZRSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZPFZRSeed_deprecated()) {
                    LogPrintf("%s: Updated zpfzr seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zpfzr seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zpfzr wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzpfzrWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZPFZRCount(nCountLastUsed);
    else if (!walletdb.ReadZPFZRCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzpfzrWallet::Lock()
{
    seedMaster = 0;
}

void CzpfzrWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzpfzrWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZPFZR(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzpfzrWallet::LoadMintPoolFromDB()
{
...
void CzpfzrWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzpfzrWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzpfzrWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zpfzrTracker->HasPubcoinHash(pMint.first)) {
...
bool CzpfzrWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZPFZR(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zpfzrTracker which also adds to database
    pwalletMain->zpfzrTracker->Add(dMint, true);
...
void CzpfzrWallet::SeedToZPFZR(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzpfzrWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzpfzrWallet::GenerateDeterministicZPFZR(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzpfzrWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzpfzrWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
