...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZPFZRStake(spend));
    } else {
        ...
        CPFZRStake* PFZRInput = new CPFZRStake();
        PFZRInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(PFZRInput);
    }
