...
enum AvailableCoinsType {
    ...
    EXCEPT_DENOMINATED_OR_MASTERNODE_COLLATERAL = 4, // ONLY_NONDENOMINATED and not 15000 PFZR at the same time
    ONLY_MASTERNODE_COLLATERAL = 5,       // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zpfzr send
enum ZerocoinSpendStatus {
    ZPFZR_SPEND_OKAY = 0,                            // No error
    ZPFZR_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZPFZR_WALLET_LOCKED = 2,                         // Wallet was locked
    ZPFZR_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZPFZR_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZPFZR_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZPFZR_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZPFZR_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZPFZR_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZPFZR_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZPFZR_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZPFZR_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZPFZR_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZPFZR_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZPFZR_SPENT_USED_ZPFZR = 14,                      // Coin has already been spend
    ZPFZR_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZPFZR_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZPFZRBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZPFZROutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzpfzrAuto) const;
    ...
    */
    ...
    CzpfzrWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzpfzrTracker> zpfzrTracker;
    ...
    void setZWallet(CzpfzrWallet* zwallet)
    {
        zwalletMain = zwallet;
        zpfzrTracker = std::unique_ptr<CzpfzrTracker>(new CzpfzrTracker(strWalletFile));
    }

    CzpfzrWallet* getZWallet() { return zwalletMain; }
    ...
    /** zpfzr reset */
    boost::signals2::signal<void()> NotifyzpfzrReset;
