...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::PFZR);
    ...
    if (!settings.contains("nAnonymizePfzerCoinAmount"))
        settings.setValue("nAnonymizePfzerCoinAmount", 1000);

    nAnonymizePfzerCoinAmount = settings.value("nAnonymizePfzerCoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizePfzerCoinAmount"))
        SoftSetArg("-anonymizepfzercoinamount", settings.value("nAnonymizePfzerCoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in pfzercoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizePfzerCoinAmount:
            return QVariant(nAnonymizePfzerCoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizePfzerCoinAmount:
            nAnonymizePfzerCoinAmount = value.toInt();
            settings.setValue("nAnonymizePfzerCoinAmount", nAnonymizePfzerCoinAmount);
            emit anonymizePfzerCoinAmountChanged(nAnonymizePfzerCoinAmount);
            break;
        case CoinControlFeatures:
