...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::PFZR, nValue);
...
static std::string makeHTMLTable(const std::string* pCells, int nRows, int nColupfzr)
{
    std::string Table = table;
    for (int i = 0; i < nRows; i++)
        Table += makeHTMLTableRow(pCells + i * nColupfzr, nColupfzr);
    Table += "</table>";
    return Table;
}
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", true)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (pfzercoin.conf).");
            QMessageBox::warning(this, "PfzerCoin Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
