...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered pfzercoin address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 PFZR.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal pfzercoin:URI (pfzercoin:XyZ...?message=example)
            {
...
static void NotifyzpfzrReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzpfzrReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzpfzrReset.connect(boost::bind(NotifyzpfzrReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzpfzrReset.disconnect(boost::bind(NotifyzpfzrReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zpfzrTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
