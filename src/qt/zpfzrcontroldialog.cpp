...
#include "ui_zpfzrcontroldialog.h"
...
std::set<std::string> ZPFZRControlDialog::setSelectedMints;
std::set<CMintMeta> ZPFZRControlDialog::setMints;

ZPFZRControlDialog::ZPFZRControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZPFZRControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZPFZRControlDialog::~ZPFZRControlDialog()
{
    delete ui;
}

void ZPFZRControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZPFZRControlDialog::updateList()
{
...
void ZPFZRControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZPFZRControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZPFZR_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZPFZRControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZPFZRControlDialog::GetSelectedMints()
{
...
void ZPFZRControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
