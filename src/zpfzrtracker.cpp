...
CzpfzrTracker::CzpfzrTracker(std::string strWalletFile)
{
...
CzpfzrTracker::~CzpfzrTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzpfzrTracker::Init()
{
...
bool CzpfzrTracker::Archive(CMintMeta& meta)
{
...
bool CzpfzrTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzpfzrTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzpfzrTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzpfzrTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzpfzrTracker::GetSerialHashes()
{
...
CAmount CzpfzrTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzpfzrTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzpfzrTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzpfzrTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzpfzrTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzpfzrTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzpfzrTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzpfzrTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzpfzrTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzpfzrTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzpfzrTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzpfzrTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzpfzrTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzpfzrTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzpfzrTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzpfzrTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzpfzrTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzpfzr from DB\n", __func__, listDeterministicDB.size());
    }
