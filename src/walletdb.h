...
public:
    ...
    bool WriteZPFZRSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZPFZRSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZPFZRSeed_deprecated(uint256& seed);
    bool EraseZPFZRSeed();
    bool EraseZPFZRSeed_deprecated();

    bool WriteZPFZRCount(const uint32_t& nCount);
    bool ReadZPFZRCount(uint32_t& nCount);
