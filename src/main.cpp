...
#if defined(NDEBUG)
#error "PfzerCoin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uPFZR) are considered zero fee (for relaying and mining)
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
	...
	if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
		return error("%s : zpfzr spend with serial %s is already in block %d\n", __func__,
...
bool ContextualCheckZerocoinSpendNoSerialCheck(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
	...
	//Check to see if the zpfzr is properly signed
	if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
		if (!spend.HasValidSignature())
			return error("%s: V2 zpfzr spend does not have a valid signature", __func__);

		libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
		if (tx.IsCoinStake())
			expectedType = libzerocoin::SpendType::STAKE;
		if (spend.getSpendType() != expectedType) {
			return error("%s: trying to spend zpfzr without the correct spend type. txid=%s", __func__,
				tx.GetHash().GetHex());
		}
	}
	...
	if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
		...
		return error("%s : zpfzr spend with serial %s from tx %s is not in valid range\n", __func__,
...

	{
		...
		if (tx.IsZerocoinSpend()) {
			...
			if (IsTransactionInChain(tx.GetHash(), nHeightTx))
				return state.Invalid(error("AcceptToMemoryPool : zpfzr spend tx %s already in block %d",
			...
			for (const CTxIn& txIn : tx.vin) {
				...
				if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
					return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
						tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zpfzr");
			}
		}
		else {
			LOCK(pool.cs);
			...
			// Check that zpfzr mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
	...
	else if (nHeight <= 7000 && nHeight > Params().LAST_POW_BLOCK()) {
		nSubsidy = 800 * COIN;
	}
	else if (nHeight <= 14000 && nHeight > 7000) {
		nSubsidy = 850 * COIN;
	}
	else if (nHeight <= 25000 && nHeight > 14000) {
		nSubsidy = 700 * COIN;
	}
	else if (nHeight <= 32000 && nHeight > 25000) {
		nSubsidy = 650 * COIN;
	}
	else if (nHeight <= 40000 && nHeight > 32000) {
		nSubsidy = 550 * COIN;
	}
	...
	else if (nHeight <= 90000 && nHeight > 70000) {
		nSubsidy = 400 * COIN;
	}
	else if (nHeight <= 77000000 && nHeight > 90000) {
		nSubsidy = 300 * COIN;
	}
	else {
		nSubsidy = 1* COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZPFZRStake)
{
    int64_t ret = 0;
...
int64_t GetMasternodePayment (int nHeight, unsigned int mnLevel, int64_t blockValue, int nMasternodeCount, bool isZPFZRStake) {
  return GetMasternodePayment (nHeight, blockValue, nMasternodeCount, isZPFZRStake) * Params ().getMasternodePhaseWeight (mnLevel, nHeight) / Params ().getMasternodePhaseWeight (0, nHeight);
}
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
	...
	for (int i = block.vtx.size() - 1; i >= 0; i--) {
		...
		* note we only undo zerocoin databasing in the following statement, value to and from PfzerCoin
...
{
	RenameThread("pfzercoin-scriptch");
	scriptcheckqueue.Thread();
}

void RecalculateZPFZRMinted()
{
...
void RecalculateZPFZRSpent()
{
	...
	while (true) {
		...
		//Remove spends from zpfzr supply
...
bool RecalculatePFZRSupply(int nHeightStart)
{
...
{
	// PfzerCoin: recalculate Accumulator Checkpoints that failed to database properly
...
bool UpdateZPFZRSupply(const CBlock& block, CBlockIndex* pindex)
{
	std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
	...
	if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
		RecalculateZPFZRMinted();
		RecalculateZPFZRSpent();
		RecalculatePFZRSupply(Params().Zerocoin_StartHeight());
	}

	//Track zpfzr money supply in the block index
	if (!UpdateZPFZRSupply(block, pindex))
		return state.DoS(100, error("%s: Failed to calculate new zpfzr supply for block=%s height=%d", __func__,
	...
	//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zPFZRSpent: %s\n",
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
	...
	for (const CTransaction& tx : block.vtx) {
		...
		// double check that there are no double spent zpfzr spends in this block
		if (tx.IsZerocoinSpend()) {
			for (const CTxIn& txIn : tx.vin) {
				if (txIn.scriptSig.IsZerocoinSpend()) {
					libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
					if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
						return state.DoS(100, error("%s : Double spending of zpfzr serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
	if (nHeight < Params().Zerocoin_Block_V2_Start())
		return error("%s: zpfzr stake block is less than allowed start height", __func__);

	if (CZPFZRStake* zpfzr = dynamic_cast<CZPFZRStake*>(stake)) {
		CBlockIndex* pindexFrom = zpfzr->GetIndexFrom();
		if (!pindexFrom)
			return error("%s: failed to get index associated with zpfzr stake checksum", __func__);

		if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
			return error("%s: zpfzr stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
		...
		uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zpfzr->GetValue()));
		if (nChecksum200 != zpfzr->GetChecksum())
			return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zpfzr->GetChecksum(), nChecksum200);
	}
	else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
	...
	if (block.IsProofOfStake()) {
		...
		if (stake->IsZPFZR() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
			return state.DoS(100, error("%s: staked zpfzr fails context checks", __func__));
	...
	if (isPoS) {
		...
		// Inputs
		std::vector<CTxIn> PFZRInputs;
		std::vector<CTxIn> zpfzrInputs;

		for (CTxIn stakeIn : stakeTxIn.vin) {
			if (stakeIn.scriptSig.IsZerocoinSpend()) {
				zpfzrInputs.push_back(stakeIn);
			}
			else {
				PFZRInputs.push_back(stakeIn);
			}
		}
		const bool hasPFZRInputs = !PFZRInputs.empty();
		const bool hasZPFZRInputs = !zpfzrInputs.empty();
		...
		for (CTransaction tx : block.vtx) {
			for (CTxIn in : tx.vin) {
				...
				if (hasPFZRInputs)
					...
					for (CTxIn PFZRIn : PFZRInputs) {
						if (PFZRIn.prevout == in.prevout) {
		...
		if (isBlockFromFork) {
			...
			// Now that this loop if completed. Check if we have zpfzr inputs.
			if (hasZPFZRInputs) {
				for (CTxIn zPFZRInput : zpfzrInputs) {
					CoinSpend spend = TxInToZerocoinSpend(zPFZRInput);
					...
					if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, spend, pindex, 0))
						return state.DoS(100, error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
							stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zpfzr");
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
	...
	if (nMints || nSpends)
		LogPrintf("%s : block contains %d zpfzr mints and %d zpfzr spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
	...
	if (strCommand == "version") {
		...
		// PfzerCoin: We use certain sporks during IBD, so check to see if they are
...
int ActiveProtocol()
{
	// SPORK_14 was used for 90053 (v1.0.0+)
	     //if (IsSporkActive(SPORK_14_NEW_PROTOCOL_ENFORCEMENT))
	        //return MIN_PEER_PROTO_VERSION_AFTER_ENFORCEMENT;

	// SPORK_15 is used for 90054 (v2.0.0+)
